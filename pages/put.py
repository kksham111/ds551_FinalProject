...
def main():
    ...
    with c_up:
        with st.form("myForm"):
            ...
            filePath = st.text_input('Enter the file name:','/',key = "FILEPATH")
            #Input for csv file
       ...
       # Storing even files in Firebase and odd files in mongoDB
        if uploadedFileCount%2 == 0:
            ...
            for i in keys:
                ...
                # getting the ID of the LastNode.
                lastNode_ID = int(json.loads(requests.get(baseURL2+"/Metadata/Lastnode.json").text))
                res1 = requests.patch(baseURL2+"/Metadata/"+fileName[0]+str(i+1)+".json", '{"id":'+str(lastNode_ID+1)+', "type": "FILE","previousNode":'+str(lastNode_ID)+'}')
                ##update the last node
                res2 = requests.patch(baseURL2+"/Metadata/.json", '{"Lastnode":'+str(lastNode_ID+1)+'}')
             
                ##Data
                res3 =requests.patch(baseURL2+"/Data/"+joinedStr+"/.json", '{"'+fileName[0]+str(i+1)+'":'+dataframe[r].to_json(orient='records')+'}')
             
                #print(baseURL2+"/Data/"+joinedStr+"/"+fileName[0]+str(i+1)+"/"+".json");
         ...
         #print(len(data))  
            ...
            ##################### ADD FILE TO MONGODB ####################
            #updating the location info on Firebase. (Contains which database and how many partitions the file is stored in)
